...
@Slf4j
@StateMachine(from = "INITIALIZED", to = "STOPPED", on = "TEST_EVENT",machineId = ConstantString.COLA_STATE_MACHINE_DEMO_ID)
@Component
public class Init2StopTransition implements Action<StatusEnum, EventEnum, DemoSpringModel> {

    @Override
    public void execute(StatusEnum from, StatusEnum to, EventEnum event, DemoSpringModel context) {
        log.info("Init2StopTransition execute, from: {}, to: {}, event: {}, context: {}", from, to, event, context);
    }
}
